#!/usr/bin/env bash
# Add bouquet.dojopop.live to the dojopop-relay Cloudflare Tunnel.
#
...
echo "==> Updating tunnel ${TUNNEL_ID} ingress (+ ${BOUQUET_HOST})..."
HTTP=$(curl -sS -o /tmp/cf-bouquet-tunnel.json -w "%{http_code}" \
  -X PUT \
...
if [[ "$HTTP" != "200" ]]; then
  ...
  python3 -m json.tool /tmp/cf-bouquet-tunnel.json || cat /tmp/cf-bouquet-tunnel.json
  exit 1
fi
...
echo "==> DNS: bouquet → ${TARGET} (zone ${DOJOPOP_ZONE_ID})"
curl -sS -X POST "https://api.cloudflare.com/client/v4/zones/${DOJOPOP_ZONE_ID}/dns_records" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"type\":\"CNAME\",\"name\":\"bouquet\",\"content\":\"${TARGET}\",\"proxied\":true}" \
  | python3 -c "import sys,json; d=json.load(sys.stdin); errs=d.get('errors') or []; e=errs[0] if errs else {}; print('  bouquet:', 'ok' if d.get('success') else e.get('message','failed'))"

echo "==> Public URL: https://${BOUQUET_HOST}"
