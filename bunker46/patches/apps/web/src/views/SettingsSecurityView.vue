...
type PasskeyRegistrationMode = 'security-key' | 'platform';
const passkeyPendingOptions = ref<PublicKeyCredentialCreationOptionsJSON | null>(null);
const passkeyPendingMode = ref<PasskeyRegistrationMode | null>(null);
const passkeyConfirmStep = ref(false);
...
async function initTotpSetup() {
  ...
  try {
    ...
    totpError.value = authErrorMessage(err, 'Failed to start TOTP setup');
  }
}
...
function isWebKitPasskeyBrowser(): boolean {
...
function passkeyErrorMessage(
  ...
  if (/timed out|timeout/i.test(msg)) {
    if (mode === 'security-key') {
      return (
        'Security key registration timed out. Safari often hangs on USB YubiKey — try Google Chrome ' +
        'on this Mac, or reboot and retry once. If Chrome is unavailable, use TOTP to sign in.'
      );
    }
    return 'Passkey registration timed out. Stay on this tab and try again.';
  }
  if (/unauthorized/i.test(msg)) {
    return 'Your session expired. Log out, sign in again, then retry.';
  }
  if (/not allowed by the user agent|not allowed|NotAllowedError/i.test(msg)) {
    if (isWebKitPasskeyBrowser()) {
      if (mode === 'platform') {
        return (
          'Safari blocked or cancelled passkey registration. Click Continue, approve with Touch ID ' +
          '(or choose your Mac login password), and do not switch away from Safari while the dialog is open.'
        );
      }
      return (
        'Safari blocked or cancelled security key registration. Plug in your YubiKey USB, click ' +
        'Continue, and if Touch ID appears choose Other Options → Security Key (or USB security key), ' +
        'then touch the key when it blinks. Stay on this tab until registration finishes.'
      );
    }
    return (
      'WebAuthn was blocked or cancelled. Plug in your YubiKey, click Register Security Key, ' +
      'choose Security Key when prompted, then touch the key when it blinks.'
    );
  }
  if (/ConstraintError|not supported/i.test(msg)) {
    return 'This authenticator is not supported for registration. Try a FIDO2 YubiKey (5 series or Security Key).';
  }
  return msg;
}
...
function withPasskeyTimeout<T>(promise: Promise<T>, mode: PasskeyRegistrationMode): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = window.setTimeout(() => {
      reject(
        new Error(
          mode === 'security-key'
            ? 'Security key registration timed out'
...
function finishPasskeyRegistration(options: PublicKeyCredentialCreationOptionsJSON) {
  const mode = passkeyPendingMode.value ?? 'security-key';
  passkeyStatus.value =
    mode === 'security-key'
      ? 'Waiting for your security key — touch the YubiKey when it blinks. Stay on this tab.'
      : 'Waiting for Touch ID — approve when Safari prompts you.';
  passkeyLoading.value = true;
  // Call startRegistration without await first — Safari/WebKit requires a direct user-gesture chain.
  withPasskeyTimeout(startRegistration({ optionsJSON: options }), mode)
    .then((attResp) => api.post<{ verified: boolean }>('/auth/passkey/register/verify', attResp))
    .then(async (result) => {
      if (result.verified) {
        passkeyStatus.value = 'Passkey registered successfully!';
        passkeyConfirmStep.value = false;
        passkeyPendingOptions.value = null;
        passkeyPendingMode.value = null;
        await loadPasskeys();
      } else {
    ...
    .catch((err) => {
      passkeyStatus.value = passkeyErrorMessage(err, mode);
    })
    .finally(() => {
...
async function preparePasskeyRegistration(mode: PasskeyRegistrationMode) {
  passkeyStatus.value = '';
  passkeyLoading.value = true;
  try {
    const options = await api.get<PublicKeyCredentialCreationOptionsJSON>(
      `/auth/passkey/register/options?mode=${mode}`,
    );
    if (isWebKitPasskeyBrowser()) {
      passkeyPendingOptions.value = options;
      passkeyPendingMode.value = mode;
      passkeyConfirmStep.value = true;
    } else {
      passkeyPendingMode.value = mode;
      finishPasskeyRegistration(options);
      return;
    }
  } catch (err) {
    passkeyStatus.value = passkeyErrorMessage(err, mode);
...
function confirmPasskeyRegistration() {
  const options = passkeyPendingOptions.value;
  if (!options) {
    passkeyStatus.value = 'Registration session expired. Click Register again.';
    passkeyConfirmStep.value = false;
    return;
  }
  finishPasskeyRegistration(options);
...
function passkeyDeleteConfirmMessage(): string {
  const isLast = passkeys.value.length === 1;
  if (isLast && totpEnabled.value) {
    return (
      'This is your only passkey. After removal you must sign in with your password AND your ' +
      'authenticator app (TOTP). Test password + TOTP login in a private window before removing ' +
      'this passkey. Continue?'
    );
  }
  if (isLast) {
    return (
      'This is your only passkey. After removal you can still sign in with username and password. ' +
      'Continue?'
    );
  }
...
async function deletePasskey(id: string) {
  if (!confirm(passkeyDeleteConfirmMessage())) return;
  try {
    await api.delete(`/auth/passkey/${id}`);
    await loadPasskeys();
    passkeyStatus.value = 'Passkey removed.';
  } catch (err) {
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8">
      <div class="space-y-6 max-w-lg">
        ...
        <Card>
          ...
          <p class="text-sm text-muted-foreground mb-4">
            Register a YubiKey or Touch ID passkey to sign in without a password. Use desktop
            Safari, Chrome, or Firefox at
            <span class="font-mono text-xs">bunker.dojopop.live</span> — in-app browsers (e.g.
            Primal) do not support WebAuthn. For USB YubiKey, Chrome on macOS is most reliable;
            Safari may hang after touch (known Apple bug).
          </p>
          ...
          <div v-if="passkeyConfirmStep" class="rounded-lg border border-border bg-muted/30 p-4 mb-4">
            <p class="text-sm font-medium mb-2">
              {{
                passkeyPendingMode === 'platform'
                  ? 'Ready to register Touch ID on this Mac'
                  : 'Ready to register your security key'
              }}
            </p>
            <p class="text-sm text-muted-foreground mb-3">
              <template v-if="passkeyPendingMode === 'platform'">
                Click Continue and approve with Touch ID when Safari prompts you.
              </template>
              <template v-else>
                Plug in your YubiKey USB. Click Continue — Safari may ask for Touch ID first;
                choose <span class="font-medium">Other Options</span> →
                <span class="font-medium">Security Key</span>, then touch when it blinks. If nothing
                happens after touch, use Chrome on this Mac instead.
              </template>
            </p>
            <div class="flex gap-2">
              <Button :loading="passkeyLoading" @click="confirmPasskeyRegistration">
                Continue
              </Button>
              <Button variant="ghost" :disabled="passkeyLoading" @click="cancelPasskeyRegistration">
                Cancel
              </Button>
            </div>
          </div>
          ...
          <div v-else class="flex flex-col gap-2 sm:flex-row">
            <Button
              ...
              @click="preparePasskeyRegistration('security-key')"
            >
            ...
            <Button
              variant="outline"
              :loading="passkeyLoading"
              @click="preparePasskeyRegistration('platform')"
            >
              Register Passkey (this device)
            </Button>
          </div>
          <p
            v-if="passkeyStatus"
            ...
            :class="
              passkeyStatus.includes('success')
                ...
                : passkeyStatus.includes('Waiting')
